...
function Build-NetFramework {
	...
	if ($NoMsbuild) {
		dotnet build -v:m -c $configuration -f $netframework_tfm -p:PublishReadyToRun=true
		if ($LASTEXITCODE) { exit $LASTEXITCODE }
	}
	else {
		msbuild -v:m -m -restore -t:Build -p:Configuration=$configuration -p:TargetFramework=$netframework_tfm -p:PublishReadyToRun=true
		if ($LASTEXITCODE) { exit $LASTEXITCODE }
	}
