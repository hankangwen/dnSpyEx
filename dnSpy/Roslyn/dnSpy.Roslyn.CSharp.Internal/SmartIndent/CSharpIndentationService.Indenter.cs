...
namespace dnSpy.Roslyn.Internal.SmartIndent.CSharp {
	internal partial class CSharpIndentationService {
		internal class Indenter : AbstractIndenter {
			public Indenter(Document document,
			...
			protected override IndentationResult? GetDesiredIndentationWorker(SyntaxToken token, TextLine previousLine,
				int lastNonWhitespacePosition) {
				...
				if (token.Span.Contains(lastNonWhitespacePosition)) {
					...
					var trivia = Tree.GetRoot(CancellationToken)
									 .FindTrivia(firstNonWhitespacePosition.Value, findInsideTrivia: true);
					if (trivia.IsKind(SyntaxKind.None) || this.LineToBeIndented.LineNumber > previousLine.LineNumber + 1) {
						...
						if (containingStatement != null && containingStatement.GetFirstToken() == token) {
							...
							return IndentFromStartOfLine(
								Finder.GetIndentationOfCurrentPosition(Tree, token, position, CancellationToken));
						}
			...
			private IndentationResult? GetIndentationBasedOnToken(SyntaxToken token) {
				...
				if (token.IsSemicolonOfEmbeddedStatement() ||
					...
					return GetIndentationOfLine(
						sourceText.Lines.GetLineFromPosition(embeddedStatementOwner.GetFirstToken(includeZeroWidth: true)
																				   .SpanStart));
				}
				...
				case SyntaxKind.ColonToken: {
					...
					if (nonTerminalNode is SwitchLabelSyntax) {
						return GetIndentationOfLine(
							...
							OptionSet.GetOption(FormattingOptions.IndentationSize, token.Language));
					}
				...
				case SyntaxKind.CloseBracketToken: {
					...
					if (nonTerminalNode is AttributeListSyntax) {
						return GetIndentationOfLine(
							sourceText.Lines.GetLineFromPosition(nonTerminalNode.GetFirstToken(includeZeroWidth: true)
																				.SpanStart));
					}
			...
			private IndentationResult? GetIndentationFromCommaSeparatedList<T>(SeparatedSyntaxList<T> list, SyntaxToken token)
				where T : SyntaxNode {
			...
			private SyntaxNode GetQueryExpressionClause(SyntaxToken token) {
				var clause = token.GetAncestors<SyntaxNode>()
								  .FirstOrDefault(n => n is QueryClauseSyntax || n is SelectOrGroupClauseSyntax);
			...
			private IndentationResult? GetDefaultIndentationFromTokenLine(SyntaxToken token, int? additionalSpace = null) {
				...
				if (nonExpressionNode == null) {
					...
					return IndentFromStartOfLine(
						Finder.GetIndentationOfCurrentPosition(Tree, token, position, spaceToAdd, CancellationToken));
				}
