...
namespace dnSpy.Decompiler.ILSpy.Settings {
	sealed class CSharpDecompilerSettingsPage : AppSettingsPage, IAppSettingsPage2 {
		...
		public void OnApply(IAppRefreshSettings appRefreshSettings) {
			...
			if (g.NativeIntegers != d.NativeIntegers) flags |= RefreshFlags.CSharp;
			if (g.NumericIntPtr != d.NumericIntPtr) flags |= RefreshFlags.CSharp;
			if (g.CovariantReturns != d.CovariantReturns) flags |= RefreshFlags.CSharp;
			if (g.InitAccessors != d.InitAccessors) flags |= RefreshFlags.CSharp;
			if (g.RecordClasses != d.RecordClasses) flags |= RefreshFlags.CSharp;
			if (g.RecordStructs != d.RecordStructs) flags |= RefreshFlags.CSharp;
			if (g.WithExpressions != d.WithExpressions) flags |= RefreshFlags.ILAst;
			if (g.UsePrimaryConstructorSyntax != d.UsePrimaryConstructorSyntax) flags |= RefreshFlags.CSharp;
			if (g.FunctionPointers != d.FunctionPointers) flags |= RefreshFlags.ILAst;
			if (g.ScopedRef != d.ScopedRef) flags |= RefreshFlags.CSharp;
			if (g.RequiredMembers != d.RequiredMembers) flags |= RefreshFlags.CSharp;
			if (g.SwitchExpressions != d.SwitchExpressions) flags |= RefreshFlags.ILAst;
			if (g.FileScopedNamespaces != d.FileScopedNamespaces) flags |= RefreshFlags.CSharp;
			if (g.AnonymousMethods != d.AnonymousMethods) flags |= RefreshFlags.ILAst | RefreshFlags.ShowMember;
			if (g.AnonymousTypes != d.AnonymousTypes) flags |= RefreshFlags.CSharp | RefreshFlags.ShowMember;
			if (g.UseLambdaSyntax != d.UseLambdaSyntax) flags |= RefreshFlags.CSharp;
			...
			if (g.Dynamic != d.Dynamic) flags |= RefreshFlags.ILAst |  RefreshFlags.ShowMember;
			if (g.AsyncAwait != d.AsyncAwait) flags |= RefreshFlags.ILAst | RefreshFlags.ShowMember;
			if (g.AwaitInCatchFinally != d.AwaitInCatchFinally) flags |= RefreshFlags.ILAst;
			if (g.AsyncEnumerator != d.AsyncEnumerator) flags |= RefreshFlags.ILAst | RefreshFlags.ShowMember;
			if (g.DecimalConstants != d.DecimalConstants) flags |= RefreshFlags.CSharp;
			if (g.FixedBuffers != d.FixedBuffers) flags |= RefreshFlags.CSharp | RefreshFlags.ShowMember;
			if (g.StringConcat != d.StringConcat) flags |= RefreshFlags.CSharp;
			if (g.LiftNullables != d.LiftNullables) flags |= RefreshFlags.ILAst;
			if (g.NullPropagation != d.NullPropagation) flags |= RefreshFlags.ILAst;
			if (g.AutomaticProperties != d.AutomaticProperties) flags |= RefreshFlags.CSharp | RefreshFlags.ShowMember;
			if (g.GetterOnlyAutomaticProperties != d.GetterOnlyAutomaticProperties) flags |= RefreshFlags.CSharp | RefreshFlags.ShowMember;
			if (g.AutomaticEvents != d.AutomaticEvents) flags |= RefreshFlags.CSharp | RefreshFlags.ShowMember;
			if (g.UsingStatement != d.UsingStatement) flags |= RefreshFlags.ILAst;
			if (g.UseEnhancedUsing != d.UseEnhancedUsing) flags |= RefreshFlags.CSharp;
			if (g.AlwaysUseBraces != d.AlwaysUseBraces) flags |= RefreshFlags.CSharp;
			if (g.ForEachStatement != d.ForEachStatement) flags |= RefreshFlags.CSharp;
			if (g.ForEachWithGetEnumeratorExtension != d.ForEachWithGetEnumeratorExtension) flags |= RefreshFlags.CSharp;
			if (g.LockStatement != d.LockStatement) flags |= RefreshFlags.ILAst;
			if (g.SwitchStatementOnString != d.SwitchStatementOnString) flags |= RefreshFlags.ILAst | RefreshFlags.ShowMember;
			if (g.SparseIntegerSwitch != d.SparseIntegerSwitch) flags |= RefreshFlags.ILAst;
			if (g.UsingDeclarations != d.UsingDeclarations) flags |= RefreshFlags.CSharp;
			if (g.ExtensionMethods != d.ExtensionMethods) flags |= RefreshFlags.CSharp;
			if (g.QueryExpressions != d.QueryExpressions) flags |= RefreshFlags.CSharp;
			if (g.UseImplicitMethodGroupConversion != d.UseImplicitMethodGroupConversion) flags |= RefreshFlags.CSharp;
			if (g.AlwaysCastTargetsOfExplicitInterfaceImplementationCalls != d.AlwaysCastTargetsOfExplicitInterfaceImplementationCalls) flags |= RefreshFlags.CSharp;
			if (g.AlwaysQualifyMemberReferences != d.AlwaysQualifyMemberReferences) flags |= RefreshFlags.CSharp;
			if (g.AlwaysShowEnumMemberValues != d.AlwaysShowEnumMemberValues) flags |= RefreshFlags.CSharp;
			if (g.UseDebugSymbols != d.UseDebugSymbols) flags |= RefreshFlags.ILAst;
			if (g.ArrayInitializers != d.ArrayInitializers) flags |= RefreshFlags.ILAst | RefreshFlags.ShowMember;
			if (g.ObjectOrCollectionInitializers != d.ObjectOrCollectionInitializers) flags |= RefreshFlags.ILAst;
			if (g.DictionaryInitializers != d.DictionaryInitializers) flags |= RefreshFlags.ILAst;
			if (g.ExtensionMethodsInCollectionInitializers != d.ExtensionMethodsInCollectionInitializers) flags |= RefreshFlags.ILAst;
			if (g.UseRefLocalsForAccurateOrderOfEvaluation != d.UseRefLocalsForAccurateOrderOfEvaluation) flags |= RefreshFlags.ILAst;
			if (g.RefExtensionMethods != d.RefExtensionMethods) flags |= RefreshFlags.CSharp;
			if (g.StringInterpolation != d.StringInterpolation) flags |= RefreshFlags.ILAst;
			if (g.ShowXmlDocumentation != d.ShowXmlDocumentation) flags |= RefreshFlags.CSharp;
			if (g.DecompileMemberBodies != d.DecompileMemberBodies) flags |= RefreshFlags.CSharp;
			if (g.UseExpressionBodyForCalculatedGetterOnlyProperties != d.UseExpressionBodyForCalculatedGetterOnlyProperties) flags |= RefreshFlags.CSharp;
			if (g.OutVariables != d.OutVariables) flags |= RefreshFlags.CSharp;
			if (g.Discards != d.Discards) flags |= RefreshFlags.CSharp;
			if (g.IntroduceRefModifiersOnStructs != d.IntroduceRefModifiersOnStructs) flags |= RefreshFlags.ILAst;
			if (g.IntroduceReadonlyAndInModifiers != d.IntroduceReadonlyAndInModifiers) flags |= RefreshFlags.CSharp;
			if (g.ReadOnlyMethods != d.ReadOnlyMethods) flags |= RefreshFlags.CSharp;
			if (g.AsyncUsingAndForEachStatement != d.AsyncUsingAndForEachStatement) flags |= RefreshFlags.ILAst;
			if (g.IntroduceUnmanagedConstraint != d.IntroduceUnmanagedConstraint) flags |= RefreshFlags.CSharp;
			if (g.StackAllocInitializers != d.StackAllocInitializers) flags |= RefreshFlags.ILAst;
			if (g.PatternBasedFixedStatement != d.PatternBasedFixedStatement) flags |= RefreshFlags.ILAst;
			if (g.TupleTypes != d.TupleTypes) flags |= RefreshFlags.ILAst;
			if (g.ThrowExpressions != d.ThrowExpressions) flags |= RefreshFlags.ILAst;
			if (g.NamedArguments != d.NamedArguments) flags |= RefreshFlags.ILAst;
			if (g.NonTrailingNamedArguments != d.NonTrailingNamedArguments) flags |= RefreshFlags.CSharp;
			if (g.OptionalArguments != d.OptionalArguments) flags |= RefreshFlags.CSharp;
			if (g.LocalFunctions != d.LocalFunctions) flags |= RefreshFlags.ILAst | RefreshFlags.ShowMember;
			if (g.Deconstruction != d.Deconstruction) flags |= RefreshFlags.ILAst;
			if (g.PatternMatching != d.PatternMatching) flags |= RefreshFlags.ILAst;
			if (g.StaticLocalFunctions != d.StaticLocalFunctions) flags |= RefreshFlags.ILAst;
			if (g.Ranges != d.Ranges) flags |= RefreshFlags.ILAst;
			if (g.NullableReferenceTypes != d.NullableReferenceTypes) flags |= RefreshFlags.CSharp;
			if (g.AssumeArrayLengthFitsIntoInt32 != d.AssumeArrayLengthFitsIntoInt32) flags |= RefreshFlags.ILAst;
			...
			if (g.RemoveDeadCode != d.RemoveDeadCode) flags |= RefreshFlags.ILAst;
			if (g.RemoveDeadStores != d.RemoveDeadStores) flags |= RefreshFlags.ILAst;
			if (g.ForStatement != d.ForStatement) flags |= RefreshFlags.ILAst;
			if (g.DoWhileStatement != d.DoWhileStatement) flags |= RefreshFlags.ILAst;
			if (g.SeparateLocalVariableDeclarations != d.SeparateLocalVariableDeclarations) flags |= RefreshFlags.CSharp;
			if (g.AggressiveScalarReplacementOfAggregates != d.AggressiveScalarReplacementOfAggregates) flags |= RefreshFlags.ILAst;
			if (g.AggressiveInlining != d.AggressiveInlining) flags |= RefreshFlags.ILAst;
			if (g.AlwaysUseGlobal != d.AlwaysUseGlobal) flags |= RefreshFlags.CSharp;
			if (g.RemoveEmptyDefaultConstructors != d.RemoveEmptyDefaultConstructors) flags |= RefreshFlags.CSharp;
			if (g.ShowTokenAndRvaComments != d.ShowTokenAndRvaComments) flags |= RefreshFlags.CSharp;
			...
			if (g.MaxArrayElements != d.MaxArrayElements) flags |= RefreshFlags.ILAst;
